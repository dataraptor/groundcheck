...
atomic claims, judges each one **against the source only** as
...
verdicts agree with human labels (precision, recall, F1, Cohen's κ). The reliability of
the tool is *shown*, not asserted.

> Faithfulness is not truth. groundcheck judges *"is this grounded in the provided
> source"* (the RAG-grounding contract), not *"is this true in the world."* A statement
> that is true in reality but absent from the source is `NOT_ENOUGH_INFO` **by design**.

## The money demo

Paste a source and an AI answer, hit **Check**, and every sentence lights up: **green**
(grounded), **amber** (not in the source), or **red** (contradicted), with the supporting
span, a one-line rationale, and a per-claim vote. The fabricated sentences turning amber
on screen *is* the pitch.
...
Open the URL, click **Check faithfulness**, and you get 62% with three amber marks. The
web demo runs on a deterministic `MockProvider`, so it needs no key and is byte-stable.
(The same frame is asserted in CI by `e2e/test_money_demo.py`.)
...
- **`api`** is a *thin* adapter: validate, call `check()`, map errors to clean JSON. It
  serves the static `app/` **same-origin** under `/app`, so the page `fetch`es `/check`
  with no CORS.
...
- **`eval`** is the meta-eval. It imports `core` directly, delegates **all** metric math to
...
# 4. the real path (needs a key: either ANTHROPIC_API_KEY, or Azure OpenAI creds in
...
`faithfulness_score` (`None` when there are 0 checkable claims, shown as *"N/A"* rather
than a misleading 100%), counts, and honest `cost_usd` / `latency_s`.

**Docker.** `docker compose up --build` builds a slim non-root image (engine + API +
static app) and serves it at <http://localhost:8080/> in **mock mode**: the 62% demo with
no key. To drive a live model instead, set `GROUNDCHECK_LLM` (`openai` for Azure, or
`anthropic`) and pass credentials by uncommenting the `env_file:` lines in
...
Generated from a single real eval run (`eval/runs/leaderboard.jsonl`, gpt-5.5, n=3, R=3).
...
| frozen | 9 | n/a | 1.000 ± 0.000 | 1.000 ± 0.000 | n/a *(n≈9, wide interval)* |
...
> no-leakage sanity check with a wide interval, not a precise number). The honest takeaway:
> on clear-cut grounding decisions the judge is reliable and its run-to-run variance is low
> (±0.000 over 3 repeats); harder, real-world, multi-annotator data would expose gaps these
> sets do not. The one visible trade-off is **Tier-2 recall at the loose τ=0.8 (0.75
> held-in)**: relaxing the threshold lets a mildly-unfaithful answer slip through, which is
> exactly why **τ=1.0 is the operating point**.

**How to read it.** Tier 1 isolates the grounding judge on fixed `{source, claim,
gold_label}` triples (no decomposition, so detector labels align 1:1 with gold), and is
...
non-deterministic, so every figure is **mean ± spread over R=3 repeats**, never a single
number pretending to be exact. The **frozen** slice (~20%, never tuned against) is reported
as **accuracy + κ only**; at n≈9 the per-class F1 is too noisy to headline.
...
into the judge (1:1 aligned, measuring the judge, the hard part), and Tier 2 measures the
whole pipeline at the answer level. Decomposition variance is folded into Tier 2 *by
design*, where it belongs, instead of silently corrupting a claim-level number.
...
**3. A refusal silently looks like a grounding failure.** Benign medical or
security-adjacent claims sometimes trip a model's safety classifier. A refusal is caught
and mapped to `NOT_ENOUGH_INFO` (never a crash), but that *lowers* the score exactly like a
genuine "source is silent" verdict. So groundcheck surfaces `n_refused` (and a per-claim
`refused` flag), and the demo and leaderboard **annotate a refusal-affected score** instead
of passing it off as a clean grounding result.

**4. `answer.find(sentence)` silently drops highlights.** Models rarely echo whitespace and
punctuation byte-for-byte, so exact-match highlighting leaves real answers unmarked.
groundcheck locates each sentence with a tolerant ladder (exact, then a `\s+`-joined token
regex that tolerates whitespace and preserves original offsets, then a ~40-char prefix,
then give up), and a sentence that still can't be located is listed as *unlocated* and
shown in the claims table **unhighlighted**. This "soft link" degrades to *"claim shown,
not highlighted,"* never to a wrong verdict.
...
| cost | ~$0.08 (≈6-claim answer) to ~$0.17 (≈11-claim demo) | **≈ $5.65** observed (gpt-5.5 estimate pricing) |
| latency | ~20-25 s (ThreadPool, 6 workers) | tens of minutes (Tier-1 grounding runs sequentially) |

- **Per full eval (observed this run):** **1,422** model calls: ~405 Tier-1 grounding calls
  (45 triples × n3 × R3) plus Tier-2 (54 checks × ~6 claims × n3 ≈ 963 groundings + 54
  decompose). At the measured per-call rate (~$0.004/grounding, gpt-5.5 estimate pricing)
  that is **≈ $5.65**. Use `--quick` (n1/R1, ~1/9th) while iterating.
- **Cache caveat:** the Opus 4.8 prompt-cache floor is **4096 tokens**, so the small demo
  sources (~90 tokens) don't cache (`cache_creation_input_tokens: 0`). This is expected,
...
- **Single-annotator gold.** Labels were authored and reviewed by one person, so κ measures
  **detector ↔ author** agreement, *not* inter-annotator reliability.
...
- **Frozen slice is small (n≈9).** It is a credible no-leakage check, but its accuracy and κ
...
- **Precision watch-out at τ=1.0.** A faithful paraphrase that adds any unstated-but-true
  detail yields an NEI claim, so `score < 1.0` and it is flagged (a false positive). The τ
  sweep makes this trade-off visible.
- **English only (v1).** The rubric and gold sets are English; multilingual is a real fork,
  not a translation.
- **Non-goals.** Not a retriever (the source is *given*), not a corrector (it flags, it
  doesn't rewrite), no world-knowledge fact-checking, single source document only.
...
- **The numbers above come from a single persisted run:** `eval/runs/leaderboard.jsonl`
  (gitignored; header + per-item predictions + summary). Its `run_header` records
  `prompt_version`, both model IDs, `n`, `R`, and `mock_mode`; one `prediction` line per
  item; a final `summary` line. That header alone reproduces the run.
- **Config that produced them:** `prompt_version = v3`, decompose `claude-sonnet-4-6`,
  ...
  `R = 3`.
- **Tests.** Full no-key suite via `./scripts/test.sh` (or `pwsh scripts/test.ps1`): the
  `core`, `api`, and `eval` unit/contract tests, plus a real-browser Playwright e2e and an
  axe-core a11y audit when chromium is available. The real-key tests skip cleanly without a
  key.
